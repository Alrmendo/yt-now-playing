...
export default function IdleState({ layout, theme, onSelectDefault }: IdleStateProps) {
  ...
  if (layout === "slim") {
    return (
      <div 
        id="widget-idle-slim"
        className="w-full flex items-center justify-between px-4 py-3 bg-slate-900/90 backdrop-blur-md rounded-xl border border-slate-800 text-xs text-slate-300 font-sans shadow-lg transition-all"
      >
        ...
        {onSelectDefault && (
          <button
            id="idle-slim-load-btn"
            onClick={onSelectDefault}
            className={`px-2.5 py-1 text-[10px] rounded-md font-medium transition-colors ${t.bgLight} ${t.text} hover:bg-opacity-20 border ${t.border}`}
          >
            Simulate Playback
          </button>
        )}
  ...
  if (layout === "compact") {
    return (
      ...
      >
        ...
        {onSelectDefault && (
          <button
            id="idle-compact-load-btn"
            onClick={onSelectDefault}
            className={`w-full py-1.5 text-xs font-medium rounded-lg transition-all ${t.bg} text-white shadow-sm hover:brightness-110 active:scale-95`}
          >
            Simulate Playback
          </button>
        )}
    ...
    >
      ...
      <div className="flex flex-col items-center space-y-4 my-auto">
        ...
        <div className="space-y-1 max-w-[200px]">
          <h3 className="text-sm font-bold text-slate-100">Awating Stream...</h3>
          <p className="text-xs text-slate-400">
            The widget remains standby until a YouTube tab starts playing.
          </p>
        </div>
      </div>

      {/* Bottom Simulated Connection Button */}
      {onSelectDefault ? (
        <button
          id="idle-full-load-btn"
          onClick={onSelectDefault}
          className={`w-full py-2.5 text-xs font-semibold rounded-xl text-white transition-all shadow-md active:scale-98 ${t.bg} hover:brightness-110`}
        >
          Inject Live Demo Song
        </button>
      ) : (
